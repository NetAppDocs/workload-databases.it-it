...
---
= Crea un Microsoft SQL Server in NetApp Workload Factory per database
:allow-uri-read: 
:icons: font
:imagesdir: ./media/
...
+
Per gli account GovCloud, immettere il valore per AWS SSM Parameter ARN nel seguente formato:
`{fsx:{"username":"fsxadmin","password":"pass"}}`.

+
Il valore è fornito nel Parameter Store in Dettagli parametro nella console di gestione AWS. Il tipo di parametro AWS SSM deve essere "Stringa sicura" con il nome nel formato  `/netapp/wlmdb/<anyname>`. Ulteriori informazioni sul recupero dei parametri SSM sono disponibili qui link:https://docs.aws.amazon.com/govcloud-us/latest/UserGuide/govcloud-ssm.html["Documentazione AWS GovCloud"^].
...
+
Per gli account GovCloud, inserisci il valore ARN del parametro AWS SSM per le credenziali di Active Directory nel formato seguente: `{fsx:{"username":"fsxadmin","password":"pass"}}`. Il tipo di parametro AWS SSM deve essere "Stringa sicura" con il nome nel formato `/netapp/wlmdb/<anyname>`. Scopri di più sul recupero dei parametri SSM da link:https://docs.aws.amazon.com/govcloud-us/latest/UserGuide/govcloud-ssm.html["Documentazione AWS GovCloud"^].
...
+
Per gli account GovCloud, inserire il valore ARN del parametro AWS SSM per le credenziali FSx for ONTAP nel seguente formato: `{fsx:{"username":"fsxadmin","password":"pass"}}`. Il tipo di parametro AWS SSM deve essere "Secure String" con il nome nel formato `/netapp/wlmdb/<anyname>`. Scopri di più su come recuperare i parametri SSM da link:https://docs.aws.amazon.com/govcloud-us/latest/UserGuide/govcloud-ssm.html["Documentazione AWS GovCloud"^].

+
Un nuovo file system FSX per ONTAP può aggiungere 30 minuti o più di tempo di installazione.

... *Seleziona un FSx per ONTAP esistente*: Seleziona il nome dell'FSx per ONTAP e inserisci un nome utente e una password per il file system.
+
...
.. *Gruppo di sicurezza*: viene creato un gruppo di sicurezza del carico di lavoro per consentire le porte e i protocolli necessari per la comunicazione tra Microsoft SQL e cluster Windows sui nodi. Inoltre, è possibile selezionare uno o più gruppi di sicurezza.
...
... Inserisci il nome utente e la password per il dominio Active Directory.
+
Per gli account GovCloud, inserisci il valore ARN del parametro AWS SSM per le credenziali di Active Directory nel formato seguente: `{fsx:{"username":"fsxadmin","password":"pass"}}`. Il tipo di parametro AWS SSM deve essere "Stringa sicura" con il nome nel formato `/netapp/wlmdb/<anyname>`. Scopri di più sul recupero dei parametri SSM da link:https://docs.aws.amazon.com/govcloud-us/latest/UserGuide/govcloud-ssm.html["Documentazione AWS GovCloud"^].
...
... *Crea nuovo file FSX per ONTAP*: Inserisci nome utente e password.
+
Per gli account GovCloud, immettere il valore ARN del parametro AWS SSM per le credenziali FSx for ONTAP nel seguente formato: `{fsx:{"username":"fsxadmin","password":"pass"}}`. Il tipo di parametro AWS SSM deve essere "Secure String" con il nome nel formato `/netapp/wlmdb/<anyname>`. Per ulteriori informazioni sul recupero dei parametri SSM da link:https://docs.aws.amazon.com/govcloud-us/latest/UserGuide/govcloud-ssm.html["Documentazione AWS GovCloud"^].
