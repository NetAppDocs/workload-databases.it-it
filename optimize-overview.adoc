...
* *Monitoraggio continuo*: Registra le risorse per abilitare il monitoraggio continuo. Workload Factory analizza regolarmente l'ambiente del database, fornisce raccomandazioni continue per ottimizzare le configurazioni e risolve automaticamente i problemi.
...
Per la valutazione una tantum, è sufficiente fornire le credenziali una sola volta per le istanze di Microsoft SQL Server o per il database Oracle che si desidera analizzare.

Per il monitoraggio continuo, è necessario registrare le risorse, associare un collegamento e concedere le autorizzazioni nel proprio account AWS.
...
* *Replicazione interregionale (CRR)*
+
Abilita la replica tra regioni (CRR) per migliorare la disponibilità dei dati e le funzionalità di ripristino di emergenza per i tuoi database Oracle. La CRR contribuisce a proteggere dalle interruzioni regionali e garantisce la continuità operativa replicando i dati in posizioni geograficamente distribuite.

* *Snapshot coerenti con l'applicazione*
+
Utilizza snapshot coerenti con l'applicazione con NetApp SnapCenter per creare snapshot accurati e affidabili dei tuoi dati di volume in un momento specifico. Questo garantisce il corretto funzionamento delle tue applicazioni e la sicurezza dei tuoi dati. SnapCenter semplifica i backup e ti aiuta a ripristinare i dati in modo rapido e corretto, riducendo i tempi di inattività e proteggendo i tuoi carichi di lavoro più importanti.
