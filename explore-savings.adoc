...
. Seleziona il menu image:workloads-menu-icon.png["L'icona del menu hamburger viene utilizzata per navigare tra carichi di lavoro quali storage, eda, ai, database, vmware e amministrazione."] e quindi selezionare *Database*.
. Dal menu Databases, seleziona *Explore savings*.
. In Esplora risparmi, seleziona la scheda *SQL Server on-premises*.
. Dalla scheda on-premise di SQL Server, scaricare lo script per valutare gli ambienti SQL Server on-premise.
+
.. Scarica lo script di valutazione. Lo script è uno strumento di raccolta di dati basato su PowerShell. Raccoglie e poi carica i dati di configurazione e prestazioni di SQL Server su Workload Factory. Il migration advisor valuta i dati e pianifica la distribuzione di FSx for ONTAP per il tuo ambiente SQL Server.
+
